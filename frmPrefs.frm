...
Begin VB.Form widgetPrefs 
   Caption         =   "Panzer OHM Gauge Preferences"
   ClientHeight    =   10545
   ClientLeft      =   60
   ClientTop       =   510
   ClientWidth     =   8880
   ...
   Begin VB.Frame fraGeneral 
      ...
      Begin VB.Frame fraGeneralInner 
         ...
         Top             =   300
         Width           =   6750
         Begin VB.CommandButton btnMonitorStart 
            Caption         =   "Monitor Start"
            Enabled         =   0   'False
            Height          =   315
            Left            =   2025
            Style           =   1  'Graphical
            TabIndex        =   190
            ToolTipText     =   "Press to select a target file for OHM."
            Top             =   4665
            Width           =   1335
         End
         Begin VB.ComboBox cmbMonitoringProgramIndex 
            Height          =   315
            ItemData        =   "frmPrefs.frx":810C
            Left            =   2025
            List            =   "frmPrefs.frx":810E
         ...
         Begin VB.ComboBox cmbTickSwitchPref 
            Height          =   315
            ItemData        =   "frmPrefs.frx":8110
            Left            =   2010
            List            =   "frmPrefs.frx":8112
            Style           =   2  'Dropdown List
            TabIndex        =   169
            Top             =   975
            Width           =   4095
         ...
         Begin VB.ComboBox cmbCurrentSensor 
            Height          =   315
            ItemData        =   "frmPrefs.frx":8114
            Left            =   2025
            List            =   "frmPrefs.frx":8116
            Style           =   2  'Dropdown List
            TabIndex        =   167
            Top             =   1950
            Width           =   4095
         ...
         Begin VB.ComboBox cmbTemperatureScale 
            Height          =   315
            ItemData        =   "frmPrefs.frx":8118
            Left            =   2010
            List            =   "frmPrefs.frx":811A
            Style           =   2  'Dropdown List
            TabIndex        =   166
            Top             =   5430
            Width           =   1740
         End
         Begin VB.CommandButton btnSelectMonitor 
            Caption         =   "..."
            Height          =   315
            Left            =   5790
            Style           =   1  'Graphical
            ...
            Width           =   315
         End
         Begin VB.TextBox txtMonitoringProgram 
            Height          =   315
            Left            =   2010
            TabIndex        =   164
            Top             =   3615
         ...
         End
         Begin VB.Label lblGeneral 
            Caption         =   "Start the monitoring Program now"
            Height          =   435
            Index           =   13
            Left            =   3540
            TabIndex        =   191
         ...
         End
         Begin VB.Label lblGeneral 
            Caption         =   "Select the sensor to monitor here or by using the middle button on the gauge surround. *"
            Height          =   660
            Index           =   1
            Left            =   2040
            TabIndex        =   176
         ...
         Begin VB.Label lblGeneral 
            Alignment       =   2  'Center
            Caption         =   "Monitoring Program Location :"
            Height          =   570
            Index           =   7
            Left            =   300
            TabIndex        =   174
         ...
         End
         Begin VB.Label lblGeneral 
            Caption         =   "The location of your temperature monitoring software (Folder+ program name)"
            Height          =   510
            Index           =   8
            Left            =   2010
            TabIndex        =   173
   ...
   Begin VB.Frame fraDevelopment 
      ...
      Begin VB.Frame fraDevelopmentInner 
         ...
         Begin VB.ComboBox cmbDebug 
            Height          =   315
            ItemData        =   "frmPrefs.frx":8633
            Left            =   1530
            List            =   "frmPrefs.frx":8635
            Style           =   2  'Dropdown List
            TabIndex        =   46
...
Private pCmbMonitoringProgramIndexBalloonToolTip As String
Private pCmbTickSwitchPrefBallonToolTip As String
Private pCmbTemperatureScaleBallonToolTip As String
Private pCmbCurrentSensorBallonToolTip As String
...
Private Sub btnMonitorStart_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If gblPrefsTooltips = "0" Then CreateToolTip btnMonitorStart.hWnd, "Click here to run the monitoring program binary shown above. Note that this will create a duplicate if already running.", _
                  TTIconInfo, "Help on the Monitoring Program Start Button", , , , True
End Sub

'---------------------------------------------------------------------------------------
' Procedure : btnSelectMonitor_MouseMove
' Author    : beededea
' Date      : 01/10/2023
' Purpose   :
'---------------------------------------------------------------------------------------
...
btnSelectMonitor_MouseMove_Error:

     MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure btnSelectMonitor_MouseMove of Form widgetPrefs"
End Sub
...
Private Sub btnSelectMonitor_Click()
    ...
    Call addTargetFile(txtMonitoringProgram.Text, retFileName)
    
    If retFileName <> vbNullString Then
        txtMonitoringProgram.Text = retFileName ' strips the buffered bit, leaving just the filename
    End If
    ...
    If Not fFExists(txtMonitoringProgram.Text) Then
        answer = vbYes
        answerMsg = "Monitoring program location is not present in the defined folder, " & txtMonitoringProgram.Text & vbCrLf & " - please ensure the monitoring program is installed, running and its correct location is entered."
...
' ----------------------------------------------------------------
' Procedure Name: cmbCurrentSensor_Click
' Purpose:
' Procedure Kind: Sub
...
' Date: 13/01/2024
' ----------------------------------------------------------------
Private Sub cmbCurrentSensor_Click()
    On Error GoTo cmbCurrentSensor_Click_Error
    
    btnSave.Enabled = True ' enable the save button
    gblCurrentSensor = cmbCurrentSensor.List(cmbCurrentSensor.ListIndex)
    sPutINISetting "Software\PzOHMGauge", "currentSensor", gblCurrentSensor, gblSettingsFile
    
    overlayWidget.thisSensorNo = cmbCurrentSensor.ListIndex
    
    On Error GoTo 0
    Exit Sub

cmbCurrentSensor_Click_Error:

    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure cmbCurrentSensor_Click, line " & Erl & "."

End Sub

'---------------------------------------------------------------------------------------
' Procedure : cmbMonitoringProgramIndex_Click
' Author    : beededea
' Date      : 29/06/2025
' Purpose   :
'---------------------------------------------------------------------------------------
'
Private Sub cmbMonitoringProgramIndex_Click()
    
   On Error GoTo cmbMonitoringProgramIndex_Click_Error

    btnSave.Enabled = True ' enable the save button
    gblMonitoringProgramIndex = cmbMonitoringProgramIndex.ListIndex
    sPutINISetting "Software\PzOHMGauge", "monitoringProgramIndex", gblMonitoringProgramIndex, gblSettingsFile
    
    'overlayWidget.monitoringProgram = cmbMonitoringProgramIndex.ListIndex

   On Error GoTo 0
   Exit Sub

cmbMonitoringProgramIndex_Click_Error:

    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure cmbMonitoringProgramIndex_Click of Form widgetPrefs"
End Sub
...
Private Sub cmbTemperatureScale_Click()
    ...
    sPutINISetting "Software\PzOHMGauge", "temperatureScale", gblTemperatureScale, gblSettingsFile
...
'---------------------------------------------------------------------------------------
' Procedure : btnMonitorStart_Click
' Author    : beededea
' Date      : 13/07/2025
' Purpose   :
'---------------------------------------------------------------------------------------
'
Private Sub btnMonitorStart_Click()

    On Error GoTo btnMonitorStart_Click_Error
    ...
    answer = vbYes
    answerMsg = "Do you wish to run " & txtMonitoringProgram.Text & vbCrLf & "?"
    answer = msgBoxA(answerMsg, vbYesNo, "Running Monitoring Software", False)
    If answer = vbYes Then Call ShellExecute(Me.hWnd, "Open", txtMonitoringProgram.Text, vbNullString, App.Path, 1)

   On Error GoTo 0
   Exit Sub

btnMonitorStart_Click_Error:

    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure btnMonitorStart_Click of Form widgetPrefs"
End Sub
...
Private Sub optGaugeTooltips_Click(Index As Integer)
    ...
    If pvtPrefsStartupFlg = False Then
        ...
        sPutINISetting "Software\PzOHMGauge", "gaugeTooltips", gblGaugeTooltips, gblSettingsFile
...
Private Sub optPrefsTooltips_Click(Index As Integer)
   ...
   On Error GoTo optPrefsTooltips_Click_Error
    ...
    If pvtPrefsStartupFlg = False Then
        ...
        sPutINISetting "Software\PzOHMGauge", "prefsTooltips", gblPrefsTooltips, gblSettingsFile
   ...
   On Error GoTo cmbMultiMonitorResize_Click_Error
    ...
    If cmbMultiMonitorResize.ListIndex = 2 Then
        If prefsMonitorStruct.IsPrimary = True Then
            ...
            sPutINISetting "Software\PzOHMGauge", "gaugePrimaryHeightRatio", gblGaugePrimaryHeightRatio, gblSettingsFile
            
            'gblPrefsPrimaryHeightTwips = Trim$(cstr(widgetPrefs.Height))
            sPutINISetting "Software\PzOHMGauge", "prefsPrimaryHeightTwips", gblPrefsPrimaryHeightTwips, gblSettingsFile
        Else
            gblGaugeSecondaryHeightRatio = fGauge.gaugeForm.WidgetRoot.Zoom
            sPutINISetting "Software\PzOHMGauge", "gaugeSecondaryHeightRatio", gblGaugeSecondaryHeightRatio, gblSettingsFile
            
            'gblPrefsSecondaryHeightTwips = Trim$(cstr(widgetPrefs.Height))
            sPutINISetting "Software\PzOHMGauge", "prefsSecondaryHeightTwips", gblPrefsSecondaryHeightTwips, gblSettingsFile
        End If
    End If
...
Private Sub Form_Load()
    ...
    Me.mnuAbout.Caption = "About Panzer OHM Gauge Cairo " & gblCodingEnvironment & " widget"
...
Private Sub initialisePrefsVars()
   ...
   On Error GoTo initialisePrefsVars_Error
    ...
    pCmbMonitoringProgramIndexBalloonToolTip = vbNullString
    pCmbTickSwitchPrefBallonToolTip = vbNullString
    pCmbTemperatureScaleBallonToolTip = vbNullString
    pCmbCurrentSensorBallonToolTip = vbNullString
    pvtPrefsFormResizedByDrag = False
...
Private Sub subClassControls()
   ...
   On Error GoTo subClassControls_Error
    ...
    Else
        ...
        Call SubclassComboBox(cmbMonitoringProgramIndex.hWnd, ObjPtr(cmbMonitoringProgramIndex))
        ...
        Call SubclassComboBox(cmbCurrentSensor.hWnd, ObjPtr(cmbCurrentSensor))
    End If
...
Public Sub MouseMoveOnComboText(sComboName As String)
    ...
    Select Case sComboName
        ...
        Case "cmbMonitoringProgramIndex"
            sTitle = "Help on Monitoring Program Selection"
            sText = pCmbMonitoringProgramIndexBalloonToolTip
            If gblPrefsTooltips = "0" Then CreateToolTip cmbMonitoringProgramIndex.hWnd, sText, , sTitle, , , , True
        Case "cmbTickSwitchPref"
            sTitle = "Help on Pointer Movement Selection"
            sText = pCmbTickSwitchPrefBallonToolTip
            If gblPrefsTooltips = "0" Then CreateToolTip cmbTickSwitchPref.hWnd, sText, , sTitle, , , , True
        Case "cmbTemperatureScale"
            sTitle = "Help on Temperature Scale Selection"
            sText = pCmbTemperatureScaleBallonToolTip
            If gblPrefsTooltips = "0" Then CreateToolTip cmbTemperatureScale.hWnd, sText, , sTitle, , , , True
        Case "cmbCurrentSensor"
            sTitle = "Help on Curren Sensor Selection"
            sText = pCmbCurrentSensorBallonToolTip
            If gblPrefsTooltips = "0" Then CreateToolTip cmbCurrentSensor.hWnd, sText, , sTitle, , , , True
...
Public Sub positionPrefsMonitor()
    ...
    If gblMonitorCount > 1 And LTrim$(gblMultiMonitorResize) = "2" Then
        ...
        If prefsMonitorStruct.IsPrimary = True Then
            gblPrefsFormResizedInCode = True
            gblPrefsPrimaryHeightTwips = fGetINISetting("Software\PzOHMGauge", "prefsPrimaryHeightTwips", gblSettingsFile)
            ...
            End If
        Else
            gblPrefsSecondaryHeightTwips = fGetINISetting("Software\PzOHMGauge", "prefsSecondaryHeightTwips", gblSettingsFile)
            gblPrefsFormResizedInCode = True
...
Private Sub chkDpiAwareness_Click()
    ...
    If pvtPrefsStartupFlg = False Then ' don't run this on startup
        ...
        sPutINISetting "Software\PzOHMGauge", "dpiAwareness", gblDpiAwareness, gblSettingsFile
...
Private Sub chkWidgetHidden_Click()
   On Error GoTo chkWidgetHidden_Click_Error
    ...
    sPutINISetting "Software\PzOHMGauge", "widgetHidden", gblWidgetHidden, gblSettingsFile
...
Private Sub cmbDebug_Click()
    ...
    If cmbDebug.ListIndex = 0 Then
        txtDefaultEditor.Text = "eg. E:\vb6\Panzer-OHM-Gauge-VB6MkII\Panzer-OHM-Gauge-VB6.vbp"
...
'    gbl24HourGaugeMode = CStr(chk24HourGaugeMode.Value)
    ...
    gblCurrentSensor = cmbCurrentSensor.ListIndex
    gblSamplingInterval = LTrim$(Str$(sliSamplingInterval.Value))
    gblTemperatureScale = cmbTemperatureScale.ListIndex
    gblMonitoringProgram = txtMonitoringProgram.Text
    gblMonitoringProgramIndex = CStr(cmbMonitoringProgramIndex.ListIndex)
...
'    End If
    ...
    If gblStartup = "1" Then
        Call writeRegistry(HKEY_CURRENT_USER, "SOFTWARE\Microsoft\Windows\CurrentVersion\Run", "PanzerOHMGauge", """" & App.Path & "\" & "Panzer-OHM-Gauge-VB6.exe""")
    Else
        Call writeRegistry(HKEY_CURRENT_USER, "SOFTWARE\Microsoft\Windows\CurrentVersion\Run", "PanzerOHMGauge", vbNullString)
    End If
    ...
    If fFExists(gblSettingsFile) Then
        ...
        sPutINISetting "Software\PzOHMGauge", "showTaskbar", gblShowTaskbar, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "showHelp", gblShowHelp, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "dpiAwareness", gblDpiAwareness, gblSettingsFile
        
        
        sPutINISetting "Software\PzOHMGauge", "gaugeSize", gblGaugeSize, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "scrollWheelDirection", gblScrollWheelDirection, gblSettingsFile
                
        sPutINISetting "Software\PzOHMGauge", "widgetFunctions", gblGaugeFunctions, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "pointerAnimate", gblPointerAnimate, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "samplingInterval", gblSamplingInterval, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "currentSensor", gblCurrentSensor, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "temperatureScale", gblTemperatureScale, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "monitoringProgram", gblMonitoringProgram, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "monitoringProgramIndex", gblMonitoringProgramIndex, gblSettingsFile
        
              
        sPutINISetting "Software\PzOHMGauge", "aspectHidden", gblAspectHidden, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "widgetPosition", gblWidgetPosition, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "widgetLandscape", gblWidgetLandscape, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "widgetPortrait", gblWidgetPortrait, gblSettingsFile

        sPutINISetting "Software\PzOHMGauge", "prefsFont", gblPrefsFont, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "gaugeFont", gblGaugeFont, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "prefsFontSizeHighDPI", gblPrefsFontSizeHighDPI, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "prefsFontSizeLowDPI", gblPrefsFontSizeLowDPI, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "prefsFontItalics", gblPrefsFontItalics, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "prefsFontColour", gblPrefsFontColour, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "displayScreenFont", gblDisplayScreenFont, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "displayScreenFontSize", gblDisplayScreenFontSize, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "displayScreenFontItalics", gblDisplayScreenFontItalics, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "displayScreenFontColour", gblDisplayScreenFontColour, gblSettingsFile

        'save the values from the Windows Config Items
        sPutINISetting "Software\PzOHMGauge", "windowLevel", gblWindowLevel, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "preventDragging", gblPreventDragging, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "opacity", gblOpacity, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "widgetHidden", gblWidgetHidden, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "hidingTime", gblHidingTime, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "ignoreMouse", gblIgnoreMouse, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "multiMonitorResize", gblMultiMonitorResize, gblSettingsFile
        
        
        sPutINISetting "Software\PzOHMGauge", "startup", gblStartup, gblSettingsFile

        sPutINISetting "Software\PzOHMGauge", "enableSounds", gblEnableSounds, gblSettingsFile
'        sPutINISetting "Software\PzOHMGauge", "enableTicks", gblEnableTicks, gblSettingsFile
'        sPutINISetting "Software\PzOHMGauge", "enableChimes", gblEnableChimes, gblSettingsFile
'        sPutINISetting "Software\PzOHMGauge", "enableAlarms", gblEnableAlarms, gblSettingsFile
'        sPutINISetting "Software\PzOHMGauge", "volumeBoost", gblVolumeBoost, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "lastSelectedTab", gblLastSelectedTab, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "debug", gblDebug, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "dblClickCommand", gblDblClickCommand, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "openFile", gblOpenFile, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "defaultVB6Editor", gblDefaultVB6Editor, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "defaultTBEditor", gblDefaultTBEditor, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "gaugeHighDpiXPos", gblGaugeHighDpiXPos, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "gaugeHighDpiYPos", gblGaugeHighDpiYPos, gblSettingsFile
        
        sPutINISetting "Software\PzOHMGauge", "gaugeLowDpiXPos", gblGaugeLowDpiXPos, gblSettingsFile
        sPutINISetting "Software\PzOHMGauge", "gaugeLowDpiYPos", gblGaugeLowDpiYPos, gblSettingsFile
...
'    chkVolumeBoost.Value = Val(gblVolumeBoost)
    ...
    lblGitHub.Caption = "You can find the code for the Panzer OHM Gauge on github, visit by double-clicking this link https://github.com/yereverluvinunclebert/Panzer-OHM-Gauge"
...
'            txtPortraitYoffset.Text = fGauge.gaugeForm.Top
    ...
    If gblMonitoringProgram <> vbNullString Then
        If fFExists(gblMonitoringProgram) Then
...
'    End With
    ...
    cmbMonitoringProgramIndex.AddItem "Open Hardware Monitor", 0
    cmbMonitoringProgramIndex.ItemData(0) = 0
    cmbMonitoringProgramIndex.AddItem "Libre Hardware Monitor", 1
    cmbMonitoringProgramIndex.ItemData(1) = 1
...
Private Sub lblGitHub_dblClick()
    ...
    If answer = vbYes Then
        Call ShellExecute(Me.hWnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-OHM-Gauge-" & gblCodingEnvironment, vbNullString, App.Path, 1)
    End If
   ...
   On Error GoTo setPrefsTooltips_Error
    ...
    ' next we just do the native VB6 tooltips
     If optPrefsTooltips(1).Value = True Then
        ...
        cmbCurrentSensor.ToolTipText = "Select the sensor you wish to monitor."
        btnSelectMonitor.ToolTipText = "Open a file browser and select the path to the temperature monitoring software."
...
'        chkVolumeBoost.ToolTipText = "Sets the volume of the various sound elements, you can boost from quiet to loud."
        ...
        cmbTickSwitchPref.ToolTipText = "The movement of the hand can be set to smooth or regular flicks, the smooth movement uses slightly more CPU."
        ...
        cmbTemperatureScale.ToolTipText = "Select the temperature scale you are used to."
        cmbCurrentSensor.ToolTipText = "Select the sensor you wish to monitor."
        txtMonitoringProgram.ToolTipText = "This field displays the full path to the temperature monitoring software."
        btnSelectMonitor.ToolTipText = "Open a file browser and select the path to the temperature monitoring software."
        
        btnMonitorStart.ToolTipText = "Click here to run the monitoring program binary shown above. Note that this will create a duplicate if already running."
        
    Else
    
        sliSamplingInterval.ToolTipText = vbNullString
        cmbTemperatureScale.ToolTipText = vbNullString
        cmbCurrentSensor.ToolTipText = vbNullString
        txtMonitoringProgram.ToolTipText = vbNullString
        btnSelectMonitor.ToolTipText = vbNullString
...
'        chkEnableAlarms.ToolTipText = vbNullString
        ...
        btnMonitorStart.ToolTipText = vbNullString
...
'    widgetPrefs.sliAnimationInterval.BackColor = RGB(redC, greenC, blueC)
    ...
    sPutINISetting "Software\PzOHMGauge", "skinTheme", gblSkinTheme, gblSettingsFile
