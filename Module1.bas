...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hWnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-OHM-Gauge-" & gblCodingEnvironment & "/issues", vbNullString, App.Path, 1)
    End If
...
Public Sub readPrefsPosition()
    ...
    If gblDpiAwareness = "1" Then
        gblPrefsHighDpiXPosTwips = fGetINISetting("Software\PzOHMGauge", "formHighDpiXPosTwips", gblSettingsFile)
        gblPrefsHighDpiYPosTwips = fGetINISetting("Software\PzOHMGauge", "formHighDpiYPosTwips", gblSettingsFile)
    ...
    Else
        gblPrefsLowDpiXPosTwips = fGetINISetting("Software\PzOHMGauge", "formLowDpiXPosTwips", gblSettingsFile)
        gblPrefsLowDpiYPosTwips = fGetINISetting("Software\PzOHMGauge", "formLowDpiYPosTwips", gblSettingsFile)
...
Public Sub writePrefsPositionAndSize()
    ...
    If widgetPrefs.IsLoaded = True And widgetPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzOHMGauge", "formHighDpiXPosTwips", gblPrefsHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzOHMGauge", "formHighDpiYPosTwips", gblPrefsHighDpiYPosTwips, gblSettingsFile
        Else
            gblPrefsLowDpiXPosTwips = CStr(widgetPrefs.Left)
            gblPrefsLowDpiYPosTwips = CStr(widgetPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzOHMGauge", "formLowDpiXPosTwips", gblPrefsLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzOHMGauge", "formLowDpiYPosTwips", gblPrefsLowDpiYPosTwips, gblSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(widgetPrefs.hWnd, "open", thisCommand, "Panzer-OHM-Gauge-VB6.exe prefs", "", 1)
    Else
